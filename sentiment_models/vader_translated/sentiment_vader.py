...
VALUE_PATH = "DATA/European_twitter_sentiment_german/German_Twitter_sentiment_dev_preprocessed_vader_translated_values.json"
BELOW_NEGATIVE = -0.33
ABOVE_POSITIVE = 0.46
...
def calculate_sentiws_vader_for_tweets(file):
    ...
    for tweet in input_data:
        tweet["vader_translated_value"] = compute_sentiment(tweet)
...
def test_vader_translated_for_tweets():
    ...
    right_guesses = 0
    for tweet in input_data:
        value = tweet["vader_translated_value"]
        if value < BELOW_NEGATIVE:
            label = "neg"
        elif value > ABOVE_POSITIVE:
            label = "pos"
...
def main():
    ...
    # optimize_label_boundaries(VALUE_PATH, "vader_translated_value")
